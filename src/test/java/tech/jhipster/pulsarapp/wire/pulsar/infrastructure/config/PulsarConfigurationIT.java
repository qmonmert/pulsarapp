package tech.jhipster.pulsarapp.wire.pulsar.infrastructure.config;
...
import tech.jhipster.pulsarapp.IntegrationTest;
...
class PulsarConfigurationIT {
  ...
  void testPulsar() throws Exception {
    ...
    Consumer<String> consumer = pulsarClient.newConsumer(Schema.STRING).loadConf(consumerProps).subscribe();

    Producer<String> producer = pulsarClient.newProducer(Schema.STRING).loadConf(producerProps).create();
